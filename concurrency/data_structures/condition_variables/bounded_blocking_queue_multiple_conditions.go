...
type BoundedBlockingQueue struct {
	...
	hasCapacity *sync.Cond
}

func Constructor(capacity int) BoundedBlockingQueue {
	m := sync.Mutex{}
	hasElements := sync.NewCond(&m)
	hasCapacity := sync.NewCond(&m)
